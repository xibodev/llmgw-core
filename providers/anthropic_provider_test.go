...
// fixtureSetupToken is a synthetic token that passes setup-token validation.
func fixtureSetupToken() string { return anthropicauth.SetupTokenPrefix + strings.Repeat("a", 80) }

func anthropicAPIKey(key string) *core.Credential {
	return &core.Credential{APIKey: key, TokenType: core.TokenTypeAPIKey}
}

func anthropicSetupCredential(token string) *core.Credential {
	return &core.Credential{Token: token, TokenType: core.TokenTypeAnthropicSetupToken}
}
...
func TestAnthropicCredentialHeadersAcrossOperations(t *testing.T) {
	...
	for name, check := range map[string]struct {
		...
		"setup token":                     {credential: anthropicSetupCredential(token), authorization: "Bearer " + token, beta: true},
		"setup token given as an api key": {credential: anthropicAPIKey(token), authorization: "Bearer " + token, beta: true},
		"keyless":                         {},
	} {
		t.Run(name, func(t *testing.T) {
			...
			for _, call := range calls {
				header := call.header
				beta := strings.Contains(strings.Join(header.Values("anthropic-beta"), ","), anthropicauth.OAuthBeta)
				if header.Get("x-api-key") != check.apiKey || header.Get("Authorization") != check.authorization || beta != check.beta ||
...
// Ported from the gateway's TestAnthropicSetupTokenCompleteAccumulatesNativeStream,
// with thinking, a signature and a tool call's input in fragments.
func TestAnthropicSetupTokenInvokeAssemblesTheStream(t *testing.T) {
	t.Parallel()
	events := anthropicEvents(
		`{"type":"message_start","message":{"id":"msg_1","model":"model","role":"assistant","content":[],"usage":{"input_tokens":2}}}`,
	...
	token := fixtureSetupToken()
	response, err := newTestAnthropic(t, server).Invoke(context.Background(), anthropicMessagesRequest(
		`{"max_tokens":64,"messages":[{"role":"user","content":"hi"}]}`, anthropicSetupCredential(token)))
	if err != nil {
		t.Fatal(err)
	}
	var got, want any
	_ = json.Unmarshal(response.Body, &got)
	_ = json.Unmarshal([]byte(`{"id":"msg_1","model":"model","role":"assistant","stop_reason":"tool_use","usage":{"input_tokens":2,"output_tokens":5},"content":[`+
		`{"type":"thinking","thinking":"ponder","signature":"sig"},{"type":"text","text":"hello"},`+
		`{"type":"tool_use","id":"toolu_1","name":"lookup","input":{"q":"x"}}]}`), &want)
	if !reflect.DeepEqual(got, want) || response.ContentType != core.ContentTypeJSON {
		t.Fatalf("message = %s", response.Body)
	}
	calls := backend.take()
	if len(calls) != 1 || calls[0].body != `{"max_tokens":64,"messages":[{"content":"hi","role":"user"}],"model":"claude-fixture","stream":true}` ||
		calls[0].header.Get("Authorization") != "Bearer "+token {
		t.Fatalf("upstream = %+v", calls)
	}
}

// Ported from the gateway's TestAnthropicSetupTokenRejectsIncompleteOrMalformedStream,
// with the other ways a stream cannot make a message.
func TestAnthropicSetupTokenRefusesUnusableStreams(t *testing.T) {
	t.Parallel()
	start := `{"type":"message_start","message":{"content":[]}}`
	stop := `{"type":"message_stop"}`
	for name, stream := range map[string]string{
		"never stopped":            anthropicEvents(start),
		"not json":                 "data: not-json\n\n",
		"data after the stop":      anthropicEvents(stop, start),
		"started twice":            anthropicEvents(start, start, stop),
		"stopped with a block":     anthropicEvents(start, `{"type":"content_block_start","index":0,"content_block":{"type":"text","text":""}}`, stop),
		"invalid tool input":       anthropicEvents(start, `{"type":"content_block_start","index":0,"content_block":{"type":"tool_use","id":"t","name":"n"}}`, `{"type":"content_block_delta","index":0,"delta":{"type":"input_json_delta","partial_json":"{"}}`, `{"type":"content_block_stop","index":0}`, stop),
		"empty":                    "",
		"stop without a start":     anthropicEvents(stop),
		"oversized record":         "data: " + strings.Repeat("x", maxStreamRecordWireSize) + "\n\n",
		"an error event":           anthropicEvents(start, `{"type":"error","error":{"type":"overloaded_error","message":"busy for owner@example.test"}}`),
		"content that is not list": anthropicEvents(start, `{"type":"message_delta","delta":{"content":"text"}}`, stop),
	} {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			_, server := newAnthropicBackend(t, func(w http.ResponseWriter, _ *http.Request, _ []byte) {
			...
			_, err := newTestAnthropic(t, server).Invoke(context.Background(), anthropicMessagesRequest(`{"messages":[]}`, anthropicSetupCredential(fixtureSetupToken())))
			...
			if name == "an error event" && (!strings.Contains(failure.Message, "type=overloaded_error") || strings.Contains(failure.Message, "owner@example.test")) {
...
// A setup token's stream that breaks off may be tried again: nothing reached
// the caller, as the gateway reasons.
func TestAnthropicSetupTokenStreamThatBreaksOffIsATransportFailure(t *testing.T) {
	t.Parallel()
	_, server := newAnthropicBackend(t, func(w http.ResponseWriter, _ *http.Request, _ []byte) {
		_, _ = io.WriteString(w, anthropicEvents(`{"type":"message_start","message":{"content":[]}}`))
...
func TestAnthropicRefusalsKeepStatusAndRetryAfter(t *testing.T) {
	...
	for name, operation := range map[string]func() error{
		...
		"anthropic setup token": func() error {
			_, err := provider.Invoke(ctx, anthropicMessagesRequest(`{"messages":[]}`, anthropicSetupCredential(fixtureSetupToken())))
			return err
		},
...
func TestAnthropicRefusesBeforeSending(t *testing.T) {
	...
	for name, credential := range map[string]*core.Credential{
		...
		"setup token that is a key": anthropicSetupCredential("fixture-secret"),
		"empty setup token":         anthropicSetupCredential(""),
		"malformed setup token":     anthropicAPIKey("sk-ant-oat01-fixture-secret"),
	} {
	...
	for name, check := range map[string]struct {
		...
		"clean": {
			header: http.Header{
				...
				"X-Api-Key": {"client-key"}, "Authorization": {"Bearer client-token"},
			},
			version: "2024-01-01", beta: []string{anthropicauth.OAuthBeta, "tokens-2024", "spaced"},
		},
		"unclean version": {header: http.Header{"Anthropic-Version": {"2024\x7f"}}, version: "2023-06-01", beta: []string{anthropicauth.OAuthBeta}},
		"none":            {version: "2023-06-01", beta: []string{anthropicauth.OAuthBeta}},
	} {
		request := core.TokenCountRequest{Header: check.header, Request: anthropicMessagesRequest(
			`{"model":"alias","system":"client","messages":[{"role":"user","content":"hi"}]}`, anthropicSetupCredential(token))}
		...
		if header.Get("anthropic-version") != check.version || !reflect.DeepEqual(header.Values("anthropic-beta"), check.beta) ||
			header.Get("x-api-key") != "" || header.Get("Authorization") != "Bearer "+token {
