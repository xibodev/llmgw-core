...
func (b *anthropicRuntimeBackend) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	b.mu.Unlock()
	setupToken := strings.HasPrefix(r.Header.Get("Authorization"), "Bearer "+anthropicauth.SetupTokenPrefix)
	switch {
	case r.Header.Get("x-api-key") != "fixture-upstream-token" && !setupToken:
...
func TestRuntimeServesTheAnthropicVertical(t *testing.T) {
	...
	owner, subscriber, stale := core.Caller{ID: "owner", Kind: core.CallerHuman}, core.Caller{ID: "subscriber", Kind: core.CallerHuman}, core.Caller{ID: "stale", Kind: core.CallerHuman}
	for caller, record := range map[core.Caller]tokenstore.Record{
		owner:      core.APIKeyRecord("fixture-upstream-token"),
		subscriber: {AccessToken: setupToken, TokenType: core.TokenTypeAnthropicSetupToken},
	...
	t.Run("a setup token's message is assembled from its stream", func(t *testing.T) {
		response, err := runtime.Invoke(ctx, subscriber, "native", messages(false))
		if err != nil || !anthropicSameJSON(t, response.Body, anthropicGoldenBody) {
			t.Fatalf("response = %s, err = %v", response.Body, err)
		}
		want := anthropicRuntimeCall{
			path: "/n/v1/messages", authorization: "Bearer " + setupToken, beta: anthropicauth.OAuthBeta, version: "2023-06-01",
			body: fmt.Sprintf(anthropicUpstreamBody, true),
		}
